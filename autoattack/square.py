...
class SquareAttack():
    ...
    def margin_and_loss(self, x_0, x_1, x_ref, y):
        """
        :param y:        correct labels if untargeted else target labels
        """

        logits = self.predict(x_0, x_1, x_ref)
        xent = F.cross_entropy(logits, y, reduction='none')
        u = torch.arange(x_0, x_1, x_ref.shape[0])
    ...
    def attack_single_run(self, x_0, x_1, x_ref, y):
        with torch.no_grad():
            adv = x_0.clone()
            c, h, w = x_0.shape[1:]
            n_features = c * h * w
            n_ex_total = x_0.shape[0]
            ...
            if self.norm == 'Linf':
                x_best = torch.clamp(x_0 + self.eps * self.random_choice(
                    [x_0.shape[0], c, 1, w]), 0., 1.)
                margin_min, loss_min = self.margin_and_loss(x_best, y)
                n_queries = torch.ones(x_0.shape[0]).to(self.device)
                ...
                for i_iter in range(self.n_queries):
                    ...
                    x_curr = self.check_shape(x_0[idx_to_fool])
                ...
                delta_init = torch.zeros_like(x_0)
                s = h // 5
                sp_init = (h - s * 5) // 2
                vh = sp_init + 0
                for _ in range(h // s):
                ...
                x_best = torch.clamp(x_0 + self.normalize(delta_init
                                                          ) * self.eps, 0., 1.)
                margin_min, loss_min = self.margin_and_loss(x_best, y)
                n_queries = torch.ones(x_0.shape[0]).to(self.device)
                s_init = int(math.sqrt(self.p_init * n_features / c))
                ...
                #    ) * self.eps, 0., 1.)
                r_best = L1_projection(x_0, delta_init, self.eps * (1. - 1e-6))
                x_best = x_0 + delta_init + r_best
                margin_min, loss_min = self.margin_and_loss(x_best, y)
                n_queries = torch.ones(x_0.shape[0]).to(self.device)
                s_init = int(math.sqrt(self.p_init * n_features / c))
    ...
    def perturb(self, x_0, x_1, x_ref, y):
        """
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                x_1_to_fool = x_0[ind_to_fool].clone()
                x_ref_to_fool = x_0[ind_to_fool].clone()
                y_to_fool = y[ind_to_fool].clone()

                _, adv_curr = self.attack_single_run(x_to_fool, x_1_to_fool, x_ref_to_fool, y_to_fool)
