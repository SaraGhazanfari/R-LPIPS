...
def check_randomized(model, x_0, x_1, x, y, bs=250, n=5, alpha=1e-4, logger=None):
    acc = []
    corrcl = []
    outputs = []
    with torch.no_grad():
        for _ in range(n):
            output = model(x_0, x_1, x)
            corrcl_curr = (output == y).sum().item()
...
def check_range_output(model, x_0, x_1, x, alpha=1e-5, logger=None):
    with torch.no_grad():
        output = model(x_0, x_1, x)
...
def check_dynamic(model, x_0, x_1, x, is_tf_model=False, logger=None):
    if is_tf_model:
