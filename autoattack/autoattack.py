...
class AutoAttack():
    ...
    def get_logits(self, x0, x1, ref):
        if not self.is_tf_model:
            return self.model(x0, x1, ref)
        else:
            return self.model.predict(x0, x1, ref)

    def get_seed(self):
        return time.time() if self.seed is None else self.seed

    def run_standard_evaluation(self, x_0_orig, x_1_orig, x_ref_orig, y_orig, bs=250, return_labels=False, state_path=None):
        ...
        #     checks.check_randomized(self.get_logits, x_0[:bs].to(self.device), x_1[:bs].to(self.device),
        #                             x_ref[:bs].to(self.device), y_orig[:bs].to(self.device), bs=bs, logger=self.logger)
        n_cls = checks.check_range_output(self.get_logits, x_0_orig[:bs].to(self.device), x_1_orig[:bs].to(self.device),
                                          x_ref_orig[:bs].to(self.device), logger=self.logger)
        # checks.check_dynamic(self.model, x_0[:bs].to(self.device), x_1[:bs].to(self.device),
        #                      x_ref[:bs].to(self.device), self.is_tf_model, logger=self.logger)
        ...
        with torch.no_grad():
            # calculate accuracy
            n_batches = int(np.ceil(x_0_orig.shape[0] / bs))
            if state.robust_flags is None:
                ...
                for batch_idx in range(n_batches):
                    ...
                    x_0 = x_0_orig[start_idx:end_idx, :].clone().to(self.device)
                    x_1 = x_1_orig[start_idx:end_idx, :].clone().to(self.device)
                    x_ref = x_ref_orig[start_idx:end_idx, :].clone().to(self.device)
                ...
                robust_accuracy = torch.sum(robust_flags).item() / x_0.shape[0]
                ...
                robust_flags = state.robust_flags.to(x_0_orig.device)
                robust_accuracy = torch.sum(robust_flags).item() / x_0_orig.shape[0]
                robust_accuracy_dict = {'clean': state.clean_accuracy}
                if self.verbose:
                    self.logger.log('initial clean accuracy: {:.2%}'.format(state.clean_accuracy))
                    self.logger.log(
                        'robust accuracy at the time of restoring the state: {:.2%}'.format(robust_accuracy))

            x_adv = x_0_orig.clone().detach()
            startt = time.time()
            for attack in attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    if attack == 'apgd-ce':
                        ...
                        adv_curr = self.apgd.perturb(x_0, x_1, x_ref, y)  # cheap=True
                        ...
                        adv_curr = self.fab.perturb(x_0, x_1, x_ref, y)

                    elif attack == 'square':
                        # square
                        self.square.seed = self.get_seed()
                        adv_curr = self.square.perturb(x_0, x_1, x_ref, y)

                    elif attack == 'apgd-t':
                        # targeted apgd
                        self.apgd_targeted.seed = self.get_seed()
                        adv_curr = self.apgd_targeted.perturb(x_0, x_1, x_ref, y)  # cheap=True
                    ...
                    if self.verbose:
                        ...
                        self.logger.log('{} - {}/{} - {} out of {} successfully perturbed'.format(
                            attack, batch_idx + 1, n_batches, num_non_robust_batch, x_0.shape[0]))
            ...
            if self.verbose:
                if self.norm == 'Linf':
                    res = (x_adv - x_0_orig).abs().reshape(x_0_orig.shape[0], -1).max(1)[0]
                elif self.norm == 'L2':
                    res = ((x_adv - x_0_orig) ** 2).reshape(x_0_orig.shape[0], -1).sum(-1).sqrt()
                elif self.norm == 'L1':
                    res = (x_adv - x_0_orig).abs().reshape(x_0_orig.shape[0], -1).sum(dim=-1)
    ...
    def clean_accuracy(self, x_0, x_1, x_ref, y_orig, bs=250):
        n_batches = math.ceil(x_0.shape[0] / bs)
        acc = 0.
        for counter in range(n_batches):
            x_0 = x_0[counter * bs:min((counter + 1) * bs, x_0.shape[0])].clone().to(self.device)
            x_1 = x_1[counter * bs:min((counter + 1) * bs, x_1.shape[0])].clone().to(self.device)
        ...
        return acc.item() / x_0.shape[0]

    def run_standard_evaluation_individual(self, x_orig, x_1, x_ref, y_orig, bs=250, return_labels=False):
        if self.verbose:
        ...
        for c in l_attacks:
            ...
            if return_labels:
                ...
                adv[c] = x_adv
            if verbose_indiv:
                acc_indiv = self.clean_accuracy(x_adv, x_1, x_ref, y_orig, bs=bs)
                space = '\t \t' if c == 'fab' else '\t'
