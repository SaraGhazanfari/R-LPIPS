...
class APGDAttack():
    ...
    def attack_single_run(self, x_0, x_1, x_ref, y, x_init=None):
        if len(x_0.shape) < self.ndims:
            x_0 = x_0.unsqueeze(0)
            y = y.unsqueeze(0)

        if self.norm == 'Linf':
            t = 2 * torch.rand(x_0.shape).to(self.device).detach() - 1
            x_adv = x_0 + self.eps * torch.ones_like(x_0
                                                     ).detach() * self.normalize(t)
        elif self.norm == 'L2':
            t = torch.randn(x_0.shape).to(self.device).detach()
            x_adv = x_0 + self.eps * torch.ones_like(x_0
                                                     ).detach() * self.normalize(t)
        elif self.norm == 'L1':
            t = torch.randn(x_0.shape).to(self.device).detach()
            delta = L1_projection(x_0, t, self.eps)
            x_adv = x_0 + t + delta
        ...
        if not x_init is None:
            ...
            if self.norm == 'L1' and self.verbose:
                print('[custom init] L1 perturbation {:.5f}'.format(
                    (x_adv - x_0).abs().view(x_0.shape[0], -1).sum(1).max()))

        x_adv = x_adv.clamp(0., 1.)
        x_best = x_adv.clone()
        x_best_adv = x_adv.clone()
        loss_steps = torch.zeros([self.n_iter, x_0.shape[0]]
                                 ).to(self.device)
        loss_best_steps = torch.zeros([self.n_iter + 1, x_0.shape[0]]
                                      ).to(self.device)
        ...
        grad = torch.zeros_like(x_0)
        for _ in range(self.eot_iter):
            if not self.is_tf_model:
                with torch.enable_grad():
                    logits = self.model(x_adv, x_1, x_ref)
        ...
        step_size = alpha * self.eps * torch.ones([x_0.shape[0], *(
        ...
        if self.norm == 'L1':
            ...
            if x_init is None:
                topk = .2 * torch.ones([x_0.shape[0]], device=self.device)
                sp_old = n_fts * torch.ones_like(topk)
            else:
                topk = L0_norm(x_adv - x_0) / n_fts / 1.5
                sp_old = L0_norm(x_adv - x_0)
        ...
        u = torch.arange(x_0.shape[0], device=self.device)
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    x_adv_1 = torch.clamp(torch.min(torch.max(x_adv_1,
                                                              x_0 - self.eps), x_0 + self.eps), 0.0, 1.0)
                    ...
                    x_adv_1 = torch.clamp(x_0 + self.normalize(x_adv_1 - x_0
                                                               ) * torch.min(self.eps * torch.ones_like(x_0).detach(),
                                                                             L2_norm(x_adv_1 - x_0, keepdim=True)), 0.0,
                                          1.0)
                    x_adv_1 = x_adv + (x_adv_1 - x_adv) * a + grad2 * (1 - a)
                    x_adv_1 = torch.clamp(
                        x_0 + self.normalize(x_adv_1 - x_0) * torch.min(self.eps * torch.ones_like(x_0).detach(),
                                                                        L2_norm(x_adv_1 - x_0, keepdim=True)), 0.0, 1.0)

                elif self.norm == 'L1':
                    grad_topk = grad.abs().view(x_0.shape[0], -1).sort(-1)[0]
                    topk_curr = torch.clamp((1. - topk) * n_fts, min=0, max=n_fts - 1).long()
                    grad_topk = grad_topk[u, topk_curr].view(-1, *[1] * (len(x_0.shape) - 1))
                    sparsegrad = grad * (grad.abs() >= grad_topk).float()
                    x_adv_1 = x_adv + step_size * sparsegrad.sign() / (
                            L1_norm(sparsegrad.sign(), keepdim=True) + 1e-10)

                    delta_u = x_adv_1 - x_0
                    delta_p = L1_projection(x_0, delta_u, self.eps)
                    x_adv_1 = x_0 + delta_u + delta_p
    ...
    def perturb(self, x_0, x_1, x_ref, y=None, best_loss=False, x_init=None):
        """
        ...
        if not y is None and len(y.shape) == 0:
            x_0.unsqueeze_(0)
            y.unsqueeze_(0)
        self.init_hyperparam(x_0)

        x_0 = x_0.detach().clone().float().to(self.device)
        x_1 = x_1.detach().clone().float().to(self.device)
        x_ref = x_ref.detach().clone().float().to(self.device)
        if not self.is_tf_model:
            y_pred = self.model(x_0, x_1, x_ref)
        else:
        ...
        if not best_loss:
            ...
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    x_to_fool = x_0[ind_to_fool].clone()
                    x_1_to_fool = x_1[ind_to_fool].clone()
                    x_ref_to_fool = x_ref[ind_to_fool].clone()
                    y_to_fool = y[ind_to_fool].clone()

                    if not self.use_largereps:
                        res_curr = self.attack_single_run(x_to_fool, x_1_to_fool, x_ref_to_fool, y_to_fool)
                    else:
            ...
            adv_best = x_0.detach().clone()
            loss_best = torch.ones([x_0.shape[0]]).to(
                self.device) * (-float('inf'))
            for counter in range(self.n_restarts):
                best_curr, _, loss_curr, _ = self.attack_single_run(x_0, x_1, x_ref, y)
...
class APGDAttack_targeted(APGDAttack):
    ...
    def perturb(self, x_0, x_1, x_ref, y=None, x_init=None):
        """
        :param x:           clean images
        :param y:           clean labels, if None we use the predicted labels
        """
        ...
        for target_class in range(2, self.n_target_classes + 2):
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    if not self.is_tf_model:
                        output = self.model(x_to_fool, x_1_to_fool, x_ref_to_fool)
                    else:
